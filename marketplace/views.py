...
# Asegúrate de tener un ProfileForm en tu forms.py para el perfil
# Si no lo tienes, puedes comentar la parte del POST en editar_perfil

urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
SDK = mercadopago.SDK("APP_USR-2885162849289081-010612-228b3049d19e3b756b95f319ee9d0011-40588817")

# ==========================================
# COTIZACIÓN SOLOENVÍOS (Corregido según API Docs)
# ==========================================
def cotizar_soloenvios(request):
    ...
    try:
        ...
        if auth_res.status_code == 200:
            ...
            # Datos del paquete (puedes ajustar los valores por defecto)
            paquete = {
            ...
            res = requests.post(rates_url, json=paquete, headers=headers, verify=False)
            ...
            if res.status_code == 200:
                ...
                tarifas_finales = []
                for t in rates:
                    precio_base = float(t.get('price') or t.get('cost') or 0)
                    if precio_base > 0:
                        tarifas_finales.append({
                            'paqueteria': t.get('service_name') or t.get('provider'),
                            'precio_final': round(precio_base * 1.08, 2), # Tu 8% de comisión
                            'tiempo': t.get('delivery_days') or '3-5 días'
                        })
                return JsonResponse({'tarifas': tarifas_finales})
                
        return JsonResponse({'tarifas': [], 'error': 'No se pudo obtener el token de envío'})
    except Exception as e:
        return JsonResponse({'tarifas': [], 'error': f'Error de conexión: {str(e)}'})

# ==========================================
# PERFIL DE USUARIO (Corregido para edición)
# ==========================================
@login_required
...
def mercadopago_webhook(request):
    # Lógica simplificada de webhook
